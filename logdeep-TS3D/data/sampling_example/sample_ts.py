...
para = {
    ...
    "ts_sequence": "ts/s2s_sequence.csv",
}
...
def load_BGL():
    ...
    bgl_structured["time"] = pd.to_datetime(
        bgl_structured["time"], format="%Y-%m-%d %H:%M:%S.%f"
    )
    ...
    bgl_structured["label"] = (bgl_structured["label"] == "error")|(bgl_structured["response_times"] >1).astype(int)
    return bgl_structured


def bgl_sampling(bgl_structured):

    label_data, time_data, event_mapping_data = (
        bgl_structured["label"].values,
    ...
    BGL_sequence.to_csv(para["ts_sequence"], index=None)
